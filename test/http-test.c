...
void http_test_canParseRequestLine() {
  ...
  string_t *request = string_fromBuffer("GET /index.html HTTP/1.1");
  ...
  string_t *request1 = string_fromBuffer("POST");
...
void http_test_canGetAndSetVersion() {
  ...
  // Set the method to HEAD 4
  http_setVersion(http, string_fromBuffer("1.5"));
  // Get the method
...
void http_test_canGetAndSetHeader() {
  http_t *http = http_create();
  string_t *key = string_fromBuffer("Host");
  string_t *value = string_fromBuffer("localhost:8080");
  ...
  string_t *sameKeyAsBefore = string_fromBuffer("Host");
  string_t *newValue = string_fromBuffer("wsic.axgn.se:8443");
...
void http_test_canCreateResponseString() {
  http_t *http = http_create();
  http_setVersion(http, string_fromBuffer("1.0"));
  http_setResponseCode(http, 200);
  http_setHeader(http, string_fromBuffer("Host"), string_fromBuffer("wsic.axgn.se:8443"));
  http_setHeader(http, string_fromBuffer("Server"), string_fromBuffer("wsic"));
  http_setBody(http, string_fromBuffer("This is a test response!"));
  string_t *responsString = http_toResponseString(http);

  string_t *expectedResult = string_fromBuffer("HTTP/1.0 200 OK\r\nHost: wsic.axgn.se:8443\r\nServer: wsic\r\nContent-Length: 24\r\n\r\nThis is a test response!");
...
void http_test_canParseHttpMethod() {
  string_t *methodGet = string_fromBuffer("GET");
  string_t *methodPut = string_fromBuffer("PUT");
  string_t *methodPost = string_fromBuffer("POST");
  string_t *methodHead = string_fromBuffer("HEAD");
  string_t *methodOptions = string_fromBuffer("OPTIONS");
  string_t *methodUnknown = string_fromBuffer("CAKE");
...
void http_test_canParseHttpRequestBody() {
  http_t *http = http_create();
  string_t *body = string_fromBuffer("HTTP/1.0 200 OK\r\nHost: wsic.axgn.se:8443\r\nServer: wsic\r\n\r\nThis is a test body!");
  ssize_t offset = 58;
...
void http_test_canParseHttpHeaders() {
  ...
  string_t *header1 = string_fromBuffer("Host: wsic.axgn.se:8443");
  ...
  string_t *header2 = string_fromBuffer("Server wsic");
  ...
  string_t *header3 = string_fromBuffer("Date:1878-12-18");
...
void http_test_canParseHost() {
  ...
  string_t *key = string_fromBuffer("Host");
  string_t *value = string_fromBuffer("localhost");
  ...
  // Values to apped to header
  string_t *key1 = string_fromBuffer("Host");
  string_t *value1 = string_fromBuffer("localhost:8443");
  ...
  // Values to apped to header
  string_t *key2 = string_fromBuffer("Host");
  string_t *value2 = string_fromBuffer("localhost:123456789");
...
void http_test_canGetAndSetBody() {
  ...
  string_t *body2 = string_fromBuffer("A hard problem and the associated back door for the NTRU Public Key Cryptosystem is described and compared/contrasted with the hard problems and back doors associated to other common public key cryptosystems.");
  string_t *expectedContentLength2 = string_fromBuffer("208");
